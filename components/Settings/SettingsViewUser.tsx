...
interface SettingsViewUserProps {
  ...
  handleCancelAction: (type: any) => void;
  ...
  return (
    <div className="space-y-2 relative" ref={dropdownRef}>
      ...
      <div 
        ...
        className={`flex items-center justify-between px-5 py-3.5 bg-white border-2 rounded-2xl transition-all cursor-pointer shadow-sm ${isOpen ? 'border-indigo-500 ring-4 ring-indigo-50' : 'border-slate-100 hover:border-slate-200'} ${disabled ? 'opacity-50 cursor-not-allowed bg-slate-50' : ''}`}
      >
        <span className="text-[12px] font-black text-slate-800 uppercase tracking-tight truncate">{displayValue}</span>
...
const EditableField = ({ label, value, onChange, type = "text", readOnly = false, error }: any) => {
  ...
  return (
    <div className="space-y-2 group">
      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block ml-1">{label}</label>
      <div className={`relative flex items-center transition-all border-2 bg-white rounded-2xl px-5 py-3.5 ${error ? 'border-red-500 ring-4 ring-red-50' : (isFocused ? 'border-indigo-500 ring-4 ring-indigo-50' : 'border-slate-100')} ${readOnly ? 'bg-slate-50/50' : ''}`}>
        <input 
          type={inputType} 
          value={value || ''} 
          onChange={onChange} 
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        ...
        <div className="flex gap-3 w-full md:w-auto">
          <button onClick={onSync} disabled={isSyncing} className={`flex-1 md:flex-none px-6 py-3 text-[10px] font-black uppercase tracking-widest rounded-2xl border transition-all flex items-center justify-center gap-2 shadow-sm ${isSyncing ? 'bg-indigo-50 text-indigo-400 border-indigo-100 animate-pulse' : 'bg-white text-indigo-600 border-slate-200 hover:bg-slate-50 active:scale-95'}`}>
            {isSyncing ? '⌛ ĐANG ĐỒNG BỘ...' : '🔄 ĐỒNG BỘ'}
          </button>
          <button onClick={() => setShowAddUserModal(true)} className="flex-1 md:flex-none px-6 py-3 bg-[#111827] hover:bg-black text-white text-[10px] font-black uppercase tracking-widest rounded-2xl shadow-xl shadow-indigo-100 transition-all active:scale-95">
            + THÊM MỚI
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {activeUsers.map(user => (
          <div key={user.id} onClick={() => { setSelectedUser(user); setEditFields(user); setPhoneError(null); }} className="group flex items-center gap-4 bg-white p-5 rounded-[2rem] border border-slate-100 hover:border-indigo-100 hover:shadow-xl transition-all duration-300 cursor-pointer min-w-0">
            <div className="w-14 h-14 rounded-2xl bg-slate-50 flex items-center justify-center text-lg font-black text-slate-400 group-hover:bg-indigo-600 group-hover:text-white transition-all shadow-sm flex-shrink-0 overflow-hidden relative border border-slate-50">
              {user.image_avatar ? <img src={user.image_avatar} className="w-full h-full object-cover" /> : user.name.charAt(0)}
              <div className={`absolute bottom-1 right-1 w-3 h-3 rounded-full border-2 border-white ${user.isOnline ? 'bg-emerald-500' : 'bg-slate-300'}`} />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-[14px] font-black text-slate-800 truncate uppercase tracking-tight leading-tight">{user.name}</p>
              <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1">
                <span className="text-[9px] font-black text-indigo-500 uppercase tracking-widest">{user.role}</span>
                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">{departments.find(d => d.id === user.departmentId)?.name || 'Chưa gán'}</span>
              </div>
            </div>
          </div>
        ))}
      ...
      {showAddUserModal && createPortal(
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md z-[9999] flex items-center justify-center p-4 animate-in fade-in duration-300">
           <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-6xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-200 border border-white/20 max-h-[92vh]">
              <div className="px-10 py-7 bg-[#111827] text-white flex justify-between items-center shrink-0">
                <h2 className="text-xl font-black uppercase tracking-[0.2em]">Thêm nhân viên mới</h2>
                <button onClick={() => setShowAddUserModal(false)} className="w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-all font-bold">✕</button>
              </div>
              
              <div className="p-8 md:p-12 overflow-y-auto hide-scrollbar">
                <div className="grid grid-cols-1 lg:grid-cols-[340px_1fr] gap-12">
                  <div className="flex flex-col items-center gap-8 lg:border-r border-slate-100 lg:pr-12">
                    <div 
                      onClick={() => addAvatarInputRef.current?.click()} 
                      className="w-64 h-64 rounded-[3.5rem] bg-slate-50 border-8 border-white shadow-xl flex items-center justify-center cursor-pointer hover:scale-[1.02] transition-all group overflow-hidden relative"
                    >
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
                         <span className="text-[10px] font-black text-white uppercase tracking-widest">Chọn ảnh đại diện</span>
                      </div>
                    </div>
                    ...
                    <div className="w-full space-y-3">
                      ...
                      <div className="w-full py-5 rounded-2xl flex items-center justify-center gap-3 border-2 bg-indigo-50 border-indigo-100 text-indigo-600">
                        <div className="w-3 h-3 rounded-full bg-indigo-500 shadow-[0_0_12px_rgba(79,70,229,0.5)]" />
                        <span className="text-[12px] font-black uppercase tracking-[0.2em]">TÀI KHOẢN MỚI</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-12">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-10 pt-6 border-t border-slate-50">
              ...
              <div className="px-10 py-8 bg-slate-50 border-t border-slate-100 flex justify-end shrink-0">
                <button onClick={handleCreateUser} className="px-14 py-4 bg-indigo-600 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl hover:bg-indigo-700 shadow-xl shadow-indigo-100 transition-all active:scale-95">XÁC NHẬN TẠO</button>
              </div>
           </div>
        </div>,
        document.body
      ...
      {/* Edit User Modal (Employee Profile) */}
      {selectedUser && createPortal(
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md z-[9999] flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-6xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-200 border border-white/20 max-h-[92vh]">
            <div className="px-10 py-7 bg-[#111827] text-white flex justify-between items-center shrink-0">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-xl bg-indigo-600 flex items-center justify-center font-black text-sm shadow-lg">K</div>
                <h2 className="text-xl font-black uppercase tracking-[0.2em]">Hồ sơ nhân viên</h2>
              </div>
              <button onClick={() => { setSelectedUser(null); setPhoneError(null); }} className="w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-all font-bold">✕</button>
            </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-[340px_1fr] gap-12">
                <div className="flex flex-col items-center gap-8 lg:border-r border-slate-100 lg:pr-12">
                  ...
                  <div className="w-full space-y-3">
                    ...
                    <div className={`w-full py-5 rounded-2xl flex items-center justify-center gap-3 border-2 transition-all ${selectedUser.isOnline ? 'bg-emerald-50 border-emerald-100 text-emerald-600' : 'bg-slate-50 border-slate-100 text-slate-400'}`}>
                  ...
                  <div className="w-full p-6 bg-slate-50/50 rounded-[2.5rem] border border-slate-100">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 text-center">NGÀY THAM GIA HỆ THỐNG</p>
                    <p className="text-[13px] font-black text-slate-800 text-center uppercase tracking-tight">{formatDate(selectedUser.createdAt)}</p>
                  </div>
                </div>
            ...
            <div className="px-10 py-8 bg-white border-t border-slate-50 flex justify-between items-center shrink-0">
               <button onClick={() => setShowDeleteModal(true)} className="px-8 py-4 text-rose-500 text-[11px] font-black uppercase tracking-[0.2em] hover:bg-rose-50 rounded-2xl transition-all">XÓA NHÂN VIÊN</button>
               <button onClick={() => setShowUpdateConfirmModal(true)} className="px-16 py-4 bg-indigo-600 text-white text-[11px] font-black uppercase tracking-[0.2em] rounded-2xl hover:bg-indigo-700 shadow-xl shadow-indigo-100 transition-all active:scale-95 ml-auto">LƯU THÔNG TIN</button>
            </div>
          </div>
        </div>,
        document.body
